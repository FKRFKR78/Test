...
HRESULT OPCClient::Connect(QString s)
{
  ...
  hResult = CoInitialize(0);//创建单线程的COM对象
  if (FAILED(hResult))
    {
      if (hResult == S_FALSE)
        {
...
bool OPCClient::dataCallback()
{
    /*
    DWORD dwCancelID;
   hResult= this->_pIOPCAsyncIO2->Read(1,item->hServerHandle,1,&dwCancelID,&pErrors);
  if(FAILED(HR))
  {
      qDebug()<<"can not read：dataCallback"<<endl;
      return false;
  }
  */
   //  // 建立异步回调
